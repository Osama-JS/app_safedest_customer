...
class _MainMap extends State<MainMap> {
  ...
  Point iniLocation = Point(coordinates: Position(39.8262, 21.4225)); // (Lng, Lat)
  ...
  void _onMapCreated(MapboxMap controller) async {
    ...
    annotationManager!.tapEvents(onTap: (annotation){
      print("Marker Tapped: ${annotation.textField}");
      final index = mapController.markersMap[annotation.textField]; // Mapbox يُنشئ ID تلقائي
      if (index != null) {
  ...
  final String _mapboxAccessToken = "pk.eyJ1Ijoib3NhbWExOTk4IiwiYSI6ImNtZ280cmw1YjFwNHQya3FxZnY2cjV5cmkifQ.gugWvJf_2VRFnk-3LVaI1w";
      ...
      if (searchQuery.value.isEmpty) {
        ...
        searchResults.assignAll(
          mapController.dataList.where((item) {
            ...
            return (
                item.pickupAddress.value.toLowerCase().contains(q) ||
                    item.deliveryAddress.value.toLowerCase().contains(q) ||
                    (item.driverName?.value != null && item.driverName!.value.toLowerCase().contains(q)) ||
                    (item.driverPhone?.value != null && item.driverPhone!.value.toLowerCase().contains(q)) ||
                    (item.vehicle?.value != null && item.vehicle!.value.toLowerCase().contains(q)) ||
  ...
  Widget _buildInfo(BuildContext context) {
    bool hasDriver =mapController.dataList[mapController.tapedIndex.value]. driverName != null && mapController.dataList[mapController.tapedIndex.value].driverName!.isNotEmpty;
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      mapController.dataList[mapController.tapedIndex.value].status.value,
              ...
              _buildLocationSection(
                ...
                title: 'موقع الاستلام',
                address: mapController.dataList[mapController.tapedIndex.value].pickupAddress.value,
              ),
              const SizedBox(height: 12),
              ...
              _buildLocationSection(
                ...
                title: 'موقع التسليم',
                address: mapController.dataList[mapController.tapedIndex.value].deliveryAddress.value,
              ),
              const SizedBox(height: 12),
              ...
              if (hasDriver)
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('بيانات السائق', style: TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 6),
                    Row(
                      children: [
                        if (mapController.dataList[mapController.tapedIndex.value].driverImage != null && mapController.dataList[mapController.tapedIndex.value].driverImage!.isNotEmpty)
                          ClipOval(
                            child: Image.network(
                              mapController.dataList[mapController.tapedIndex.value].driverImage!.value,
                              width: 40,
                              height: 40,
                              fit: BoxFit.cover,
                        ...
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(mapController.dataList[mapController.tapedIndex.value].driverName!.value, style: const TextStyle(fontWeight: FontWeight.bold)),
                            if (mapController.dataList[mapController.tapedIndex.value].driverPhone != null && mapController.dataList[mapController.tapedIndex.value].driverPhone!.isNotEmpty)
                              Text(mapController.dataList[mapController.tapedIndex.value].driverPhone!.value, style: const TextStyle(color: Colors.grey)),
                          ],
                        ),
                      ],
                    ),
              ...
              // --- نوع المركبة (إن وُجد) ---
              if (mapController.dataList[mapController.tapedIndex.value].vehicle != null && mapController.dataList[mapController.tapedIndex.value].vehicle!.isNotEmpty)
                Text.rich(
                  TextSpan(
                    text: 'المركبة: ',
                    style: const TextStyle(fontWeight: FontWeight.bold),
                    children: [
                      TextSpan(text: mapController.dataList[mapController.tapedIndex.value].vehicle!.value, style: const TextStyle(fontWeight: FontWeight.normal)),
                    ],
                  ),
                ),
  ...
  Widget mapSearchWidget(){
    return Padding(
      ...
      child: Column(
        children: [
          TextField(
            decoration: customInputDecoration(null,Icons.search).copyWith(hintText: 'search'.tr,filled: true,fillColor: Colors.white,),
      ...
      Card(
        ...
        child: Padding(
            ...
            return
              InkWell(
                onTap: () {
                  ...
                  mapController.tapedIndex.value = mapController.dataList.indexOf(item);
                  mapController.showInfo.value = true;
          ...
          children: [
            ...
            Text(
              '${item.price
                  .value.toStringAsFixed(2)} ${item.currency.value}',
                  ...
                  Row(
                    children: [
                      ...
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(item.driverName!.value, style: const TextStyle(
                              fontWeight: FontWeight.bold)),
                          if (item.driverPhone != null &&item.driverPhone!.isNotEmpty)
                            Text(item.driverPhone!.value,
  ...
  Widget _buildLocationSection({required IconData icon, required String title, required String address}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
