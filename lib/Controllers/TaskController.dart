...
class TaskController extends GetxController {
  ...
  Future<List> getData() async {
    try {
      ...
      var data = await helperData.getData(
        currentPage.value,
  ...
  Future<Map<String, dynamic>> getTaskDetails(int taskId) async {
    try {
      var data = await helperData.getTaskDetails(
        taskId,
        Token_pref.getToken()!,
      );
      ...
      if (data["success"] == true) {
        ...
        return {
          "success": false,
          "message": data["message"] ?? "Failed to load task details",
        };
      }
    } catch (e) {
      return {"success": false, "message": "Error loading task details: $e"};
    }
  }
}
